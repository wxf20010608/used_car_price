...
input_path = os.path.join(project_root, "data")  # 直接使用项目根目录下的data文件夹
print(f"Looking for data files in: {input_path}")  # 添加调试信息

Train_data = pd.read_csv(os.path.join(input_path, 'used_car_train_20200313.csv'), sep=' ')
Test_data = pd.read_csv(os.path.join(input_path, 'used_car_testB_20200421.csv'), sep=' ')
...
for kind, kind_data in Train_gb:
    ...
    info['regionCode_mad'] = mad(kind_data.price)
...
for kind, kind_data in Train_gb:
    ...
    info['brand_price_mad'] = mad(kind_data.price)
...
for kind, kind_data in Train_gb:
    ...
    info['model_price_mad'] = mad(kind_data.price)
    all_info[kind] = info
...
for kind, kind_data in Train_gb:
    ...
    info['bodyType_price_mad'] = mad(kind_data.price)
...
for kind, kind_data in Train_gb:
    ...
    info['fuelType_price_mad'] = mad(kind_data.price)
...
output_path = os.path.join(project_root, "code", "user_data")
if not os.path.exists(output_path):
    os.makedirs(output_path)
tree_data = df
print(tree_data.shape)
train_num = df.shape[0]-50000
tree_data[0:int(train_num)].to_csv(os.path.join(output_path, 'train_tree.csv'), index=0,sep=' ')
tree_data[train_num:train_num+50000].to_csv(os.path.join(output_path, 'text_tree.csv'), index=0,sep=' ')
...
—————————————————————————————————————————————以下为神经网络的数据处理—————————————————————————————————————————————
"""
# 使用之前定义的input_path，不再重新定义
Train_data = pd.read_csv(os.path.join(input_path, 'used_car_train_20200313.csv'), sep=' ')
Test_data = pd.read_csv(os.path.join(input_path, 'used_car_testA_20200313.csv'), sep=' ')
...
print(nn_data.shape)
train_num = df.shape[0]-50000
nn_data[0:int(train_num)].to_csv(os.path.join(output_path, 'train_nn.csv'), index=0, sep=' ')
nn_data[train_num:train_num+50000].to_csv(os.path.join(output_path, 'test_nn.csv'), index=0, sep=' ')

print('NN模型数据已经准备完毕~~~~~~~')
#只对数值型列进行中位数填充
df.select_dtypes(include=['int64', 'float64']).fillna(df.select_dtypes(include=['int64', 'float64']).median(), inplace=True)

#对类别型特征填充众数
df.select_dtypes(include=['object']).fillna(df.select_dtypes(include=['object']).mode().iloc[0], inplace=True)
dd = 'v_3'
Train_gb = df.groupby(kk)
all_info = {}
for kind, kind_data in Train_gb:
...
feature_cols = [col for col in feature_cols if
             col  not in
             ['new14+6', 'new13+6', 'new0+12', 'new9+11', 'v_3', 'new11+10', 'new10+14', 'new12+4', 
              'new3+4', 'new11+11', 'new13+3', 'new8+1', 'new1+7', 'new11+14', 'new8+13', 'v_8', 
              'v_0', 'new3+5', 'new2+9', 'new9+2', 'new0+11', 'new13+7', 'new8+11', 'new5+12', 
              'new10+10', 'new13+8', 'new11+13', 'new7+9', 'v_1', 'new7+4', 'new13+4', 'v_7', 
              'new5+6', 'new7+3', 'new9+10', 'new11+12', 'new0+5', 'new4+13', 'new8+0', 'new0+7', 
              'new12+8', 'new10+8', 'new13+14', 'new5+7', 'new2+7', 'v_4', 'v_10', 'new4+8', 
              'new8+14', 'new5+9', 'new9+13', 'new2+12', 'new5+8', 'new3+12', 'new0+10', 'new9+0', 
              'new1+11', 'new8+4', 'new11+8', 'new1+1', 'new10+5', 'new8+2', 'new6+1', 'new2+1', 
              'new1+12', 'new2+5', 'new0+14', 'new4+7', 'new14+9', 'new0+2', 'new4+1', 'new7+11', 
              'new13+10', 'new6+3', 'new1+10', 'v_9', 'new3+6', 'new12+1', 'new9+3', 'new4+5', 
              'new12+9', 'new3+8', 'new0+8', 'new1+8', 'new1+6', 'new10+9', 'new5+4', 'new13+1', 
              'new3+7', 'new6+4', 'new6+7', 'new13+0', 'new1+14', 'new3+11', 'new6+8', 'new0+9', 
              'new2+14', 'new6+2', 'new12+12', 'new7+12', 'new12+6', 'new12+14', 'new4+10', 
              'new2+4', 'new6+0', 'new3+9', 'new2+8', 'new6+11', 'new3+10', 'new7+0', 'v_11', 
              'new1+3', 'new8+3', 'new12+13', 'new1+9', 'new10+13', 'new5+10', 'new2+2', 'new6+9', 
              'new7+10', 'new0+0', 'new11+7', 'new2+13', 'new11+1', 'new5+11', 'new4+6', 'new12+2', 
              'new4+4', 'new6+14', 'new0+1', 'new4+14', 'v_5', 'new4+11', 'v_6', 'new0+4', 
              'new1+5', 'new3+14', 'new2+10', 'new9+4', 'new2+6', 'new14+14', 'new11+6', 'new9+1', 
              'new3+13', 'new13+13', 'new10+6', 'new2+3', 'new2+11', 'new1+4', 'v_2', 'new5+13', 
              'new4+2', 'new0+6', 'new7+13', 'new8+9', 'new9+12', 'new0+13', 'new10+12', 'new5+14', 
              'new6+10', 'new10+7', 'v_13', 'new5+2', 'new6+13', 'new9+14', 'new13+9', 'new14+7', 
              'new8+12', 'new3+3', 'new6+12']]