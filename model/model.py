...
TestA_data = pd.read_csv(os.path.join(tree_data_path, 'text_tree.csv'), sep=' ')
...
for fold_, (trn_idx, val_idx) in enumerate(folds.split(X_data, Y_data)):
    ...
    num_round = 1000000
    # 添加回调函数
    callbacks = [lgb.callback.log_evaluation(period=300), lgb.callback.early_stopping(stopping_rounds=300)]
    clf = lgb.train(param, trn_data, num_round, valid_sets=[trn_data, val_data], feval=myFeval, callbacks=callbacks)
...
#导入树模型lgb预测数据
predictions_lgb = np.array(pd.read_csv(os.path.join(tree_data_path, 'test_lgb.csv'))['price'])
oof_lgb = np.array(pd.read_csv(os.path.join(tree_data_path, 'train_lgb.csv'))['price'])

#导入树模型cab预测数据
predictions_cb = np.array(pd.read_csv(os.path.join(tree_data_path, 'test_cab.csv'))['price'])
...
output_path = os.path.join(path, "code", "prediction_result")
# 测试集输出
sub = pd.DataFrame()
sub['SaleID'] = TestA_data.SaleID
predictions[predictions < 0] = 0
sub['price']=predictions
sub.to_csv(os.path.join(output_path, 'predictions.csv'), index=False)
